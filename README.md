...
## AWS autoscaling migration pack

This ZIP now includes an optional ECS/Fargate migration layer without removing the working Lightsail deployment. Start with the current Docker Compose setup, then migrate in stages:

1. Split Celery jobs into light and heavy queues. The current single Lightsail worker listens to both queues, so this does not break MVP deployment.
2. Move persistent data to PostgreSQL/RDS before running multiple API tasks.
3. Move uploads/outputs/reports to private S3 before running multiple workers.
4. Deploy web, API, light worker, heavy worker, and beat as separate ECS services behind an ALB.
5. Use ECS Service Auto Scaling for web/API CPU, memory, and request count, and CloudWatch queue-depth alarms for workers.

New documents are in `docs/aws-autoscaling-architecture.md` and the related migration guides. Template task definitions and GitHub Actions workflow are included under `infra/ecs/` and `.github/workflows/deploy-ecs.yml`.
