...
from app.core.queueing import priority_for_plan, queue_for_job
...
def create_tool_job(tool_id: str, action: str, payload: JobCreate, request: Request, user: dict | None = Depends(current_user)):
    ...
    try:
        ...
        queue_name = queue_for_job(plugin, tool_action, file_path, options)
        job['queue'] = queue_name
        st.save_job(job)
        process_tool_job.apply_async(
            args=[job["id"], plugin.definition.slug, tool_action, str(file_path), options],
            queue=queue_name,
            priority=priority_for_plan(plan_name),
        )
        return job
