...
def _api_public_base() -> str:
    # OAuth redirect URIs must use the public API/proxy URL, not a private container URL.
    # On single-domain deployments this is normally the same as FRONTEND_URL.
    return (settings.api_public_url or settings.frontend_url).rstrip('/')
...
def _provider_config(provider: str) -> dict[str, str]:
    if provider == 'google':
        return {
            ...
            'redirect_uri': settings.google_redirect_uri or f"{_api_public_base()}/api/auth/oauth/google/callback",
    ...
    if provider == 'microsoft':
        ...
        return {
            ...
            'redirect_uri': settings.microsoft_redirect_uri or f"{_api_public_base()}/api/auth/oauth/microsoft/callback",
    ...
    if provider == 'github':
        return {
            ...
            'redirect_uri': settings.github_redirect_uri or f"{_api_public_base()}/api/auth/oauth/github/callback",
