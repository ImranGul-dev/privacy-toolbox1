...
def process_tool_job(job_id: str, tool_id: str, action: ToolAction, file_path: str, options: dict[str, Any] | None = None):
    ...
    path = Path(file_path)
    job = load(job_id)
    # In ECS/S3 mode the API task and worker task do not share a local filesystem.
    # If the path from the API task is not present in this worker, rehydrate it from storage.
    if not path.exists() and job.get('file_id'):
        path = st.find_upload(str(job['file_id']))
    ...
    try:
        ...
        if action == "scan":
            ...
            token, output_s3_key = st.persist_output(job_id, out, display_filename, owner_id=str(job.get('owner_id') or ''))
            update(
                job,
                status="completed",
                progress=100,
