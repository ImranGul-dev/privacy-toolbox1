...
def find_upload(file_id: str) -> Path:
    ...
    if _use_s3() and meta.get('s3_key'):
        ...
        return _download_s3_to_cache(str(meta['s3_key']), UPLOADS / f'{validate_id(file_id, label="file id")}_{filename}')
...
def load_job(job_id: str) -> dict:
    if _use_s3():
        job = _get_json_s3(_job_key(job_id))
        if not job:
...
def make_download_token(path: Path, filename: str | None = None, *, owner_id: str = '', job_id: str = '', s3_key: str = '') -> str:
...
def persist_output(job_id: str, path: Path, filename: str, *, owner_id: str = '') -> tuple[str, str]:
    ...
    if _use_s3():
        ...
        return make_download_token(path, filename=filename, owner_id=owner_id, job_id=job_id, s3_key=key), key
    return make_download_token(path, filename=filename, owner_id=owner_id, job_id=job_id), ''
