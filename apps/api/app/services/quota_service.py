...
def _use_redis() -> bool:
    return settings.quota_backend.lower() == 'redis'
...
def _redis_check_and_consume(user: dict | None, plan_name: str, action: str, actor_override: str | None = None) -> dict[str, Any]:
    ...
    r = _redis_client()
    day_key = f'quota:day:{today}:{actor}:{kind}'
    mon_key = f'quota:month:{month}:{actor}:files'

    pipe = r.pipeline()
    pipe.incr(day_key, 1)
    pipe.expire(day_key, 60 * 60 * 48)
    pipe.incr(mon_key, 1)
    pipe.expire(mon_key, 60 * 60 * 24 * 45)
    day_count, _, month_count, _ = pipe.execute()

    if int(day_count) > day_limit or int(month_count) > plan.monthly_files:
        refund(actor, action, plan_name)
        if int(day_count) > day_limit:
    ...
    return {
        'actor': actor,
        'day': {'scan': int(r.get(f'quota:day:{today}:{actor}:scan') or 0), 'clean': int(r.get(f'quota:day:{today}:{actor}:clean') or 0)},
        'month': {'files': int(month_count)},
        'plan': plan.name,
        'backend': 'redis',
    }
...
def check_and_consume(user: dict | None, plan_name: str, action: str, actor_override: str | None = None) -> dict[str, Any]:
    if _use_redis():
        return _redis_check_and_consume(user, plan_name, action, actor_override)
    ...
    def _consume():
        ...
        data[actor] = bucket
        _save(data)
        return {'actor': actor, 'day': day, 'month': mon, 'plan': plan.name, 'backend': 'local'}
...
def refund(actor: str, action: str, plan_name: str | None = None) -> None:
    ...
    if _use_redis():
        ...
        month = now.strftime('%Y-%m')
        r = _redis_client()
        for key in [f'quota:day:{today}:{actor}:{kind}', f'quota:month:{month}:{actor}:files']:
            try:
